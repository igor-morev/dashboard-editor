...
export function testimonialsWidget(layout: TestimonialsLayout = 'grid', content?: TestimonialsContent): Widget {
  
  const layoutTransformer = (currentLayout: TestimonialsLayout, data: TestimonialsContent) => {
    
    // Хелпер для создания карточки отзыва
    const createReviewCard = (rev: TestimonialItem, classes: string) => columnWidget([
      // Рейтинг (звездочки)
  ...
  const defaultContent: TestimonialsContent = content || {
    ...
    items: [
      { author: 'Sarah Johnson', role: 'Business Owner', text: 'This platform changed the way we build websites forever.', avatar: 'https://pravatar.cc', rating: 5 },
      { author: 'Mark Smith', role: 'Developer', text: 'The cleanest code I have ever seen in a low-code tool.', avatar: 'https://pravatar.cc', rating: 5 },
      { author: 'Jane Doe', role: 'Designer', text: 'Highly recommend for anyone who values time and quality.', avatar: 'https://pravatar.cc', rating: 4 }
    ]
  };
